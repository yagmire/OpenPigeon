...
//       com/google/androidgamesdk/SwappyDisplayManager.java
//
// This is the class that actually triggers the in-memory DEX load on current
// Android. SwappyCommon::SwappyCommon() gates the ChoreographerCallback on the
// NDK Choreographer being unavailable (so it is unused from API 30 up), but it
// gates the display manager on usesMinSdkOrLater() -- SDK >= 28 -- rather than
// on useSwappyDisplayManager(), which excludes SDK >= 31. So Swappy constructs
// SwappyDisplayManager on every modern device and calls gamesdk::loadClass()
// for it on the render thread.
//
// See ChoreographerCallback.java in this package for the full explanation of
// why both classes are vendored here.
